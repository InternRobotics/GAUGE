...
const engineResults: EngineResult[] = [
  ...
  { task: "Pendulum", category: "rigid", material: "Metal", frames: "300 / 3,000", metrics: [{ label: "PD", baseline: "1.14 (0.066)", target: "one", values: [1.10, 2.47, 1.09] }, { label: "EL", baseline: "0.00", target: "zero", values: [0.034, -0.041, 0.022] }] },
...
function formatMetricValue(value: number | null, metric: EngineMetric) {
  if (value === null) return "—";
  return value.toFixed(metric.label === "EL" ? 3 : 2);
}
      ...
      <div className="gap-readout" aria-live="polite">
        ...
        <div className="gap-values">
          {heroEngineNames.map((name, index) => {
            ...
            return <div key={name} className={best ? "best" : ""}><span><i style={{ backgroundColor: heroEngineColors[index] }} />{name}</span><strong>{formatMetricValue(value, selectedMetric)}</strong><small>{best ? "closest" : gap === null ? "no rollout" : `gap ${gap.toFixed(selectedMetric.label === "EL" ? 3 : 2)}`}</small></div>;
          })}
        </div>
      </div>
